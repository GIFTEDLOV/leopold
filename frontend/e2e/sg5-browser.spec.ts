...
function safeHexChainId(chainId: number): string {
...
async function bridgeRequest(
  ...
  try {
    ...
    if (method === "eth_signTypedData_v4") {
      const payload = JSON.parse(asString(params[1])) as {
...
async function installProvider(context: BrowserContext, chainId: number = SG5_LOCKED.chainId) {
  ...
  await context.exposeFunction("__sg5ProviderRequest", (method: string, params: readonly unknown[]) =>
    bridgeRequest(wallet, rpc, method, params || [], chainId, true),
  );
  await context.exposeFunction("__sg5UnauthorizedProviderRequest", (method: string, params: readonly unknown[]) =>
    bridgeRequest(unauthorized, rpc, method, params || [], chainId, false),
  );
  await context.addInitScript((requestedChainId) => {
    const provider = (bridgeName: string) => ({
      request: ({ method, params }: { method: string; params?: readonly unknown[] }) =>
        (window as unknown as Record<string, unknown>)[bridgeName] instanceof Function
          ? (window as unknown as Record<string, (...args: unknown[]) => Promise<unknown>>)[bridgeName](
              method,
              params || [],
            )
          : Promise.reject(new Error("SG5_PROVIDER_BRIDGE_MISSING")),
      on: () => undefined,
      removeListener: () => undefined,
    });
    Object.defineProperty(window, "ethereum", { configurable: false, value: provider("__sg5ProviderRequest") });
    Object.defineProperty(window, "__sg5UnauthorizedEthereum", {
      configurable: false,
...
async function observeContext(context: BrowserContext, page: Page) {
  ...
  await context.route("**/*", async (route) => {
    try {
      classifyNetworkUrl(route.request().url(), SG5_ASSET_ORIGIN_AUTHORITY);
  ...
  page.on("request", (request) => {
    try {
      classifyNetworkUrl(request.url(), SG5_ASSET_ORIGIN_AUTHORITY);
      requestState.set(request, { started: performance.now() });
    } catch {
      errors.forbiddenNetwork += 1;
    }
  });
  ...
  page.on("response", (response) => {
    try {
      ...
      const state = requestState.get(request);
      if (!state) throw new Error("response without request");
      ...
      if (!observation.success) {
        errors.forbiddenNetwork += 1;
        process.stdout.write(`SG5_NETWORK_FAILURE:${observation.requestCategory}:${observation.statusCategory}\n`);
      }
    } catch {
      errors.forbiddenNetwork += 1;
    }
...
async function readResult(page: Page): Promise<SanitizedProbeResult> {
  ...
  if (!text) throw new Error("missing SG5 result");
...
async function waitForProbeOutcome(page: Page): Promise<void> {
  await expect
    .poll(
      async () => (await page.getByTestId("sg5-result").count()) + (await page.getByTestId("sg5-failure").count()),
      { timeout: 600_000 },
    )
    .toBe(1);
  if (await page.getByTestId("sg5-failure").count())
    throw new Error(`SG5_BROWSER_PROBE_FAILURE:${await page.getByTestId("sg5-failure-class").textContent()}`);
}
...
test.describe("SG-5 browser capability", () => {
  ...
  test("SG5-01..09 clean browser encrypted lifecycle", async ({ browser }) => {
    ...
    for (let index = 0; index < SG5_LOCKED.coldContextCount; index += 1) {
      ...
      process.stdout.write(
        `SG5_HARNESS_DIAGNOSTIC:${JSON.stringify({ errors: observed.errors, observations: observed.networkObservations.length, categories: observed.networkObservations.map((entry) => `${entry.originClassification}:${entry.requestCategory}:${entry.statusCategory}`) })}\n`,
      );
      expect(result.status).toBe("CAPABILITY_COMPLETE");
      expect(result.observedChainId).toBe("11155111");
      expect(result.transactionStatus).toBe("SUCCESS");
      expect(result.authorizedPlaintextMatched).toBe(true);
      expect(result.unauthorizedDecryptionRejected).toBe(true);
      await page.reload();
      await waitForProbeOutcome(page);
  ...
  test("SG5-04 wrong network refuses before FHE work", async ({ browser }) => {
    const context = await browser.newContext({ serviceWorkers: "block" });
    const page = await context.newPage();
    await installProvider(context, 1);
    const observed = await observeContext(context, page);
    await page.goto("/sg5-probe");
    await expect(page.getByTestId("sg5-wrong-network-refused")).toBeVisible();
    await expect(page.getByTestId("sg5-no-transaction")).toBeVisible();
    expect(observed.errors.forbiddenNetwork).toBe(0);
    expect(observed.networkObservations.some((entry) => entry.requestCategory !== "LOCAL_FRONTEND_ASSET")).toBe(false);
    scenarioResults["SG5-04_WRONG_NETWORK"] = true;
    await context.close();
  });
  ...
  test.afterAll(() => {
    ...
    const aggregate = {
      ...
      protocolVersion: "sg5-browser-capability-v2",
      status: passing === 2 && scenarioResults["SG5-04_WRONG_NETWORK"] === true ? "PASS" : "FAIL",
      executionMode: "LIVE_SEPOLIA",
      requiredColdContexts: "2",
      passedColdContexts: String(passing),
    ...
    if (process.env.SG5_RESULT_PATH)
      writeFileSync(
        process.env.SG5_RESULT_PATH,
        `${JSON.stringify({ aggregate, scenarios: scenarioResults }, null, 2)}\n`,
        { mode: 0o600 },
      );
  });
});
