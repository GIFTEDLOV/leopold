...
export default function RewardsPage() {
  ...
  const refundVault = financial.fixture && financial.enteredVaults.has("weekly")
    ? leopoldConfig.vaults.find((vault) => vault.slug === "weekly")
    : liveRefundVault;
  const refundAmount = financial.fixture && refundVault
    ...
    <div className={styles.content}>
      ...
      <div className={styles.pageHeading}>
        <div>
          ...
          <h1>Rewards</h1>
          <p>Track bond refunds and settlement rewards separately from prizes, yield, and your savings principal.</p>
        </div>
      </div>
      <div className={styles.metrics}>
        <article className={styles.metricHero}>
          ...
          <button
            ...
            onClick={() => { if (refundVault) void financial.claimRefund(refundVault.slug).catch(() => undefined); }}
          >
            Claim refund
          </button>
        </article>
        <article>
          ...
          <p>Credited only when this address performs eligible draw-finalization work.</p>
        </article>
        <article>
          ...
          <strong>—</strong>
          <p>Completed reward claims appear here separately from prize winnings.</p>
        </article>
      </div>
      <section className={styles.panel}>
        <p className={styles.eyebrow}>REWARD AVAILABILITY</p><h2>By vault.</h2>
        {leopoldConfig.vaults.map((vault) => {
          ...
          return (
            <div className={styles.listRow} key={vault.slug}>
              <div><strong>{vault.name}</strong><span>Settlement reward credit</span></div>
              <div>
                ...
                {reward && reward > 0n ? (
                  <button
                    ...
                    onClick={() => void financial.claimRewards(vault.slug).catch(() => undefined)}
                  >
                    Claim
                  </button>
                ) : null}
