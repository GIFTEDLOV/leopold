...
function RecentDraws({ state }: { state: ReturnType<typeof useV2FunctionalState> }) {
  return <div className={styles.historyList}>
    ...
    {state.data?.latestResultRound ? <div className={styles.historyRow}><div><strong>Draw #{state.data?.latestResultRoundId?.toString() ?? "—"}</strong><span>{state.resultReady ? "Your latest result is ready." : "Result will appear after settlement."}</span></div><Status tone={state.resultReady ? "good" : "neutral"}>{state.resultReady ? "Ready" : "Pending"}</Status></div> : null}
  </div>;
}
...
export function V2HomePage() {
  ...
  return <>
    ...
    {state.topologyError || (state.readStatus === "error" && !state.readTransient) ? <section className={styles.systemNotice}><div><strong>Prize Savings data needs attention.</strong><span>{state.topologyError ? "The current on-chain topology could not be verified." : state.readError}</span></div><button className={`${styles.button} ${styles.buttonSecondary}`} type="button" onClick={() => void state.load()}>Try again</button></section> : null}
