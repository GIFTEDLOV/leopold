...
export async function executeRealInstalledSdkProbe(): Promise<SafeProbeObservation> {
  ...
  try {
    const [{ ZamaSDK, createConfig, memoryStorage }, { sepolia }, { web }, { ViemProvider, ViemSigner }, viem, viemChains] = await Promise.all([import("@zama-fhe/sdk"), import("@zama-fhe/sdk/chains"), import("@zama-fhe/sdk/web"), import("@zama-fhe/sdk/viem"), import("viem"), import("viem/chains")]);
    const imports = { ZamaSDK, createConfig, memoryStorage, sepolia, web, ViemProvider, ViemSigner, viem, viemChains };
    const chainHex = await ethereum.request({ method: "eth_chainId" }); observation.observedChainId = Number(chainHex);
    ...
    const accounts = await ethereum.request({ method: "eth_requestAccounts" }); const address = requireAddress(Array.isArray(accounts) ? accounts[0] : undefined); observation.walletConnected = true;
    const { sdk, publicClient, provider, signer } = await wasmProbe.runCriticalSection("SDK_INITIALIZATION", () => makeSdk(ethereum, address, imports)); observation.sdkInstanceCreated = true;
    publicKeyMaterial = await sdk.relayer.fetchFheEncryptionKeyBytes(); const publicKey = publicKeyMaterial as { publicKeyBytes?: { bytes?: Uint8Array }; crsBytes?: { bytes?: Uint8Array } }; observation.publicKeyRetrieved = Boolean(publicKey.publicKeyBytes?.bytes instanceof Uint8Array && publicKey.publicKeyBytes.bytes.byteLength > 0 && publicKey.crsBytes?.bytes instanceof Uint8Array && publicKey.crsBytes.bytes.byteLength > 0); if (!observation.publicKeyRetrieved) throw new Error("empty official encryption material"); publicKeyMaterial = undefined;
    encryptedResult = await wasmProbe.runCriticalSection("SDK_ENCRYPTION", () => sdk.encrypt({ values: [{ value: SG5_LOCKED.value, type: SG5_LOCKED.encryptedWidth }], contractAddress: SG5_LOCKED.contractAddress as `0x${string}`, userAddress: address })); observation.encryptedInputBuilderCreated = true; const payloads = encryptedResult.encryptedValues; observation.encryptedPayloadCount = Array.isArray(payloads) ? payloads.length : 0; observation.encryptedPayloadNonempty = observation.encryptedPayloadCount === 1 && asHex(payloads[0]); observation.inputProofPresent = asHex(encryptedResult.inputProof); observation.encryptionCompleted = observation.encryptedPayloadNonempty && observation.inputProofPresent; if (!observation.encryptionCompleted) throw new Error("browser encryption returned invalid contract material");
    const txHash = await signer.writeContract({ address: SG5_LOCKED.contractAddress as `0x${string}`, abi: PROBE_ABI, functionName: "submit", args: [payloads[0] as `0x${string}`, encryptedResult.inputProof as `0x${string}`] }); observation.transactionSubmitted = true; observation.transactionHash = txHash;
    const receipt = await publicClient.waitForTransactionReceipt({ hash: txHash }); observation.transactionConfirmed = receipt.status === "success"; observation.transactionStatus = receipt.status === "success" ? "SUCCESS" : "NOT_RUN"; observation.transactionBlockNumber = String(receipt.blockNumber); if (!observation.transactionConfirmed) throw new Error("SG5_TRANSACTION_FAILED");
    handle = await provider.readContract({ address: SG5_LOCKED.contractAddress as `0x${string}`, abi: PROBE_ABI, functionName: "getValue", args: [] }); observation.resultHandleRead = asHex(handle); if (!observation.resultHandleRead) throw new Error("SG5_RESULT_HANDLE_MISSING");
    const clearValues = await wasmProbe.runCriticalSection("SDK_DECRYPTION", () => sdk.decryption.decryptValues([{ encryptedValue: handle as `0x${string}`, contractAddress: SG5_LOCKED.contractAddress as `0x${string}` }])); const clear = clearValues[handle as `0x${string}`]; observation.authorizedDecryptionCompleted = true; observation.authorizedPlaintextMatched = clear === 1n || clear === "1";
    const unauthorizedEthereum = window.__sg5UnauthorizedEthereum; if (!unauthorizedEthereum) throw new Error("SG5_UNAUTHORIZED_PROVIDER_MISSING"); observation.unauthorizedDecryptionAttempted = true; try { const unauthAccounts = await unauthorizedEthereum.request({ method: "eth_requestAccounts" }); const unauthAddress = requireAddress(Array.isArray(unauthAccounts) ? unauthAccounts[0] : undefined); const unauthorized = await makeSdk(unauthorizedEthereum, unauthAddress, imports); await unauthorized.sdk.decryption.decryptValues([{ encryptedValue: handle as `0x${string}`, contractAddress: SG5_LOCKED.contractAddress as `0x${string}` }]); observation.unauthorizedDecryptionRejected = false; } catch { observation.unauthorizedDecryptionRejected = true; }
