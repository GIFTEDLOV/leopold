...
const RESULT_KEYS = [
  ...
  "resultHandleRead",
  "authorizedDecryptionCompleted",
  "authorizedPlaintextMatched",
  "unauthorizedDecryptionAttempted",
  "unauthorizedDecryptionRejected",
...
function nonnegativeInteger(value: number): string {
  assert(Number.isSafeInteger(value) && value >= 0, "unsafe integer");
  return String(value);
}
function assertDecimal(value: unknown, field: string): asserts value is string {
  assert(typeof value === "string" && DECIMAL.test(value), `invalid decimal: ${field}`);
}
function assertBoolean(value: unknown, field: string): asserts value is boolean {
  assert(typeof value === "boolean", `invalid boolean: ${field}`);
}
function assertEnum<T extends string>(value: unknown, allowed: readonly T[], field: string): asserts value is T {
  assert(typeof value === "string" && allowed.includes(value as T), `invalid ${field}`);
}
...
function assertSafeGraph(value: unknown, path = "$", seen = new Set<object>()): void {
  ...
  if (
    ...
    throw new Error(`${path} contains raw material`);
  if (Array.isArray(value)) {
    assert(Object.getPrototypeOf(value) === Array.prototype, `${path} array prototype`);
    for (const item of value) assertSafeGraph(item, `${path}[]`, seen);
    seen.delete(value);
    return;
  }
  assert(Object.getPrototypeOf(value) === Object.prototype, `${path} must be plain`);
  for (const key of Reflect.ownKeys(value)) {
    assert(typeof key === "string", `${path} has symbol`);
    const descriptor = Object.getOwnPropertyDescriptor(value, key);
    assert(Boolean(descriptor && descriptor.enumerable && "value" in descriptor), `${path}.${key} accessor`);
...
function capabilityComplete(result: Record<string, unknown>): boolean {
  return (
    result.observedChainId === "11155111" &&
    ...
    result.transactionSubmitted === true &&
    result.transactionConfirmed === true &&
    result.transactionStatus === "SUCCESS" &&
    typeof result.transactionHash === "string" &&
    HEX32.test(result.transactionHash) &&
    typeof result.transactionBlockNumber === "string" &&
    DECIMAL.test(result.transactionBlockNumber) &&
    result.resultHandleRead === true &&
    result.authorizedDecryptionCompleted === true &&
    result.authorizedPlaintextMatched === true &&
    result.unauthorizedDecryptionAttempted === true &&
    result.unauthorizedDecryptionRejected === true
  );
}
...
function exactOrigin(value: string | null): boolean {
...
function validateAuthority(authority: Readonly<AssetOriginAuthority>): void {
  ...
  for (const prefix of [authority.publicKeyAssetPathPrefix, authority.crsAssetPathPrefix])
    assert(
      prefix !== null && prefix.startsWith("/") && !prefix.includes("?") && !prefix.includes("#"),
...
function rawAuthority(urlText: string): string {
  ...
  if (!match) throw new Error("URL authority missing");
  return match[1];
}
export function classifyNetworkUrl(
  urlText: string,
  authority: Readonly<AssetOriginAuthority> = SG5_ASSET_ORIGIN_AUTHORITY,
): { originClassification: OriginClassification; requestCategory: RequestCategory } {
  validateAuthority(authority);
  const url = new URL(urlText);
  assert(!url.username && !url.password && !url.hash, "URL credentials/fragments forbidden");
  const registered = [
    "localhost:3000",
    "relayer.testnet.zama.org",
    "ethereum-sepolia-rpc.publicnode.com",
    new URL(authority.publicKeyAssetOrigin!).host,
  ];
  assert(registered.includes(rawAuthority(urlText)), "unregistered network authority");
  assert(
    (url.origin === SG5_ORIGINS.localhost && url.protocol === "http:") ||
      (url.protocol === "https:" && url.port === ""),
    "unsupported URL scheme",
  );
  if (url.origin === SG5_ORIGINS.localhost) {
    assert(url.port === "3000", "localhost port");
    return { originClassification: "LOCALHOST", requestCategory: "LOCAL_FRONTEND_ASSET" };
  }
  if (url.origin === SG5_ORIGINS.relayer) {
    assert(url.pathname.startsWith("/v2/") && !url.search, "unregistered relayer path");
    return {
      originClassification: "OFFICIAL_RELAYER",
      requestCategory: url.pathname === "/v2/keyurl" ? "RELAYER_KEYURL_METADATA" : "RELAYER_RUNTIME",
    };
  }
  if (url.origin === SG5_ORIGINS.chainRpc) {
    assert(url.pathname === "/" && !url.search, "unregistered RPC path");
  ...
  if (url.origin === authority.publicKeyAssetOrigin) {
    ...
    if (pathMatches(url.pathname, authority.publicKeyAssetPathPrefix!))
      return { originClassification: "OFFICIAL_PUBLIC_KEY_ASSET", requestCategory: "PUBLIC_KEY_ASSET" };
    if (pathMatches(url.pathname, authority.crsAssetPathPrefix!))
...
function statusCategory(status: number): StatusCategory {
  assert(Number.isInteger(status) && status >= 200 && status <= 599, "invalid status");
  return status < 300
    ? "SUCCESS_2XX"
    : status < 400
      ? "REDIRECT_3XX"
...
export function buildResponseNetworkObservation(
  ...
  let redirect: RedirectClassification = "NONE";
  if (input.redirectedToUrl) {
    try {
      const from = classifyNetworkUrl(input.requestUrl, authority);
      const to = classifyNetworkUrl(input.redirectedToUrl, authority);
      redirect =
        from.originClassification === to.originClassification ? "ALLOWED_SAME_ORIGIN" : "ALLOWED_REGISTERED_ORIGIN";
    } catch {
      redirect = "FORBIDDEN";
    }
  }
  if (input.redirectedFromUrl) {
    try {
      const from = classifyNetworkUrl(input.redirectedFromUrl, authority);
      const to = classifyNetworkUrl(input.requestUrl, authority);
      redirect =
        from.originClassification === to.originClassification ? "ALLOWED_SAME_ORIGIN" : "ALLOWED_REGISTERED_ORIGIN";
    } catch {
      redirect = "FORBIDDEN";
  ...
  return {
    ...
    durationMilliseconds: nonnegativeInteger(Math.max(0, Math.round(input.durationMilliseconds))),
    redirectClassification: redirect,
    success,
  };
}
export function buildNetworkFailureObservation(
  ...
  if (redirectedFromUrl) {
    try {
      const from = classifyNetworkUrl(redirectedFromUrl, authority);
      redirect =
        from.originClassification === classification.originClassification
          ? "ALLOWED_SAME_ORIGIN"
          : "ALLOWED_REGISTERED_ORIGIN";
    } catch {
      redirect = "FORBIDDEN";
    }
  }
  return {
    ...
    durationMilliseconds: nonnegativeInteger(Math.max(0, Math.round(durationMilliseconds))),
    redirectClassification: redirect,
    success: false,
  };
}
...
export function hasRequiredNetworkProof(result: SanitizedProbeResult): boolean {
  const required: Array<[OriginClassification, RequestCategory]> = [
    ...
    ["OFFICIAL_RELAYER", "RELAYER_RUNTIME"],
    ["OFFICIAL_PUBLIC_KEY_ASSET", "PUBLIC_KEY_ASSET"],
    ["OFFICIAL_CRS_ASSET", "CRS_ASSET"],
    ["OFFICIAL_CHAIN_RPC", "SEPOLIA_RPC"],
  ];
  return (
    result.networkObservations.every((entry) => entry.success && entry.redirectClassification !== "FORBIDDEN") &&
    required.every(([origin, category]) =>
      result.networkObservations.some(
        (entry) => entry.originClassification === origin && entry.requestCategory === category && entry.success,
      ),
    )
  );
}
...
export function sanitizeObservation(
  ...
  const result: SanitizedProbeResult = {
    ...
    browserVersion: "RECORDED_BY_PLAYWRIGHT",
    chainId: "11155111",
    observedChainId: String(observation.observedChainId),
    sdkPackage: "@zama-fhe/sdk",
    sdkVersion: "3.4.0",
    reactSdkPackage: "@zama-fhe/react-sdk",
    reactSdkVersion: "3.4.0",
    providerReady: observation.providerReady,
    walletConnected: observation.walletConnected,
    ...
    transactionSubmitted: observation.transactionSubmitted,
    transactionConfirmed: observation.transactionConfirmed,
    transactionStatus: observation.transactionStatus,
    transactionHash: observation.transactionHash,
    transactionBlockNumber: observation.transactionBlockNumber,
    resultHandleRead: observation.resultHandleRead,
    authorizedDecryptionCompleted: observation.authorizedDecryptionCompleted,
    authorizedPlaintextMatched: observation.authorizedPlaintextMatched,
    unauthorizedDecryptionAttempted: observation.unauthorizedDecryptionAttempted,
    unauthorizedDecryptionRejected: observation.unauthorizedDecryptionRejected,
  ...
  if (
    executionMode === "LIVE_SEPOLIA" &&
    (!real || !capabilityComplete(result as unknown as Record<string, unknown>))
  ) {
    result.status = "FAIL";
    result.failureClassification = real ? "CAPABILITY_INCOMPLETE" : "CAPABILITY_INCOMPLETE";
...
export function assertSanitizedResult(value: unknown): asserts value is SanitizedProbeResult {
  ...
  assert(value !== null && typeof value === "object" && !Array.isArray(value), "result object required");
  assertExactKeys(value as object, RESULT_KEYS, "result");
  ...
  for (const [key, expected] of Object.entries({
    ...
    assert(result[key] === expected, `invalid ${key}`);
  assertEnum(result.status, ["STRUCTURAL_PASS_NOT_LIVE", "CAPABILITY_COMPLETE", "FAIL"], "status");
  assertEnum(result.executionMode, ["OFFLINE_STRUCTURAL", "LIVE_SEPOLIA"], "execution mode");
  assertEnum(result.adapterClassification, ["REAL_INSTALLED_SDK", "CONTROLLED_TEST_FAKE"], "adapter");
  assertEnum(result.blockerClassification, ["NONE", "BROWSER_UNAVAILABLE", "WRONG_NETWORK"], "blocker");
  assertEnum(
  ...
  assertEnum(result.browserEngine, ["CHROMIUM_FAMILY", "UNKNOWN_BROWSER"], "browser");
  assertEnum(result.transactionStatus, ["SUCCESS", "NOT_RUN"], "transaction status");
  assertEnum(result.finalVerdict, ["NOT_LIVE", "PASS_PENDING_HARNESS", "FAIL"], "verdict");
  assertDecimal(result.observedChainId, "observedChainId");
  assert(
    result.transactionHash === null ||
      (typeof result.transactionHash === "string" && HEX32.test(result.transactionHash)),
    "transaction hash",
  );
  assert(
    result.transactionBlockNumber === null ||
      (typeof result.transactionBlockNumber === "string" && DECIMAL.test(result.transactionBlockNumber)),
    "transaction block",
  );
  for (const key of [
    "providerReady",
  ...
  assert(Array.isArray(result.wasmCriticalSections) && result.wasmCriticalSections.length === 3, "WASM section count");
  const seen = new Set<string>();
  for (const section of result.wasmCriticalSections) {
    assertExactKeys(section, WASM_KEYS, "WASM section");
    assertEnum(
      section.criticalSectionClassification,
      ["SDK_INITIALIZATION", "SDK_ENCRYPTION", "SDK_DECRYPTION"],
      "WASM section",
    );
    assert(!seen.has(section.criticalSectionClassification), "duplicate WASM section");
    seen.add(section.criticalSectionClassification);
    assertDecimal(section.successfulInstantiateCount, "WASM success");
    assertDecimal(section.successfulInstantiateStreamingCount, "WASM streaming");
    assertDecimal(section.failedCallCount, "WASM failure");
  }
  assert(Array.isArray(result.networkObservations) && result.networkObservations.length <= 512, "network observations");
  for (const observation of result.networkObservations) {
    assertExactKeys(observation, NETWORK_KEYS, "network observation");
    assertEnum(
      observation.originClassification,
    ...
    assertDecimal(observation.durationMilliseconds, "network duration");
    assertEnum(
      observation.redirectClassification,
      ["NONE", "ALLOWED_SAME_ORIGIN", "ALLOWED_REGISTERED_ORIGIN", "FORBIDDEN"],
      "redirect",
    );
    assertBoolean(observation.success, "network success");
  }
  ...
  if (result.status === "CAPABILITY_COMPLETE") {
    assert(
      result.executionMode === "LIVE_SEPOLIA" &&
        result.adapterClassification === "REAL_INSTALLED_SDK" &&
        result.failureClassification === "NONE" &&
        result.finalVerdict === "PASS_PENDING_HARNESS",
      "live contradiction",
    );
    assert(capabilityComplete(result), "live capability incomplete");
  }
  if (result.status === "FAIL")
    assert(result.finalVerdict === "FAIL" && result.failureClassification !== "NONE", "failure contradiction");
}

export function assertSanitizedAggregateResult(value: unknown): asserts value is SanitizedAggregateResult {
  assertSafeGraph(value);
  assert(value !== null && typeof value === "object" && !Array.isArray(value), "aggregate object required");
  assertExactKeys(value as object, AGGREGATE_KEYS, "aggregate");
  const result = value as Record<string, unknown>;
  assert(
    result.schema === "zama-szn4.sg5-browser-probe-aggregate.v2" && result.protocolVersion === SG5_PROTOCOL_VERSION,
    "aggregate identity",
  );
  assert(result.status === "PASS" || result.status === "FAIL", "aggregate status");
  assert(result.executionMode === "LIVE_SEPOLIA", "aggregate mode");
  assert(result.requiredColdContexts === "2", "aggregate context count");
  assertDecimal(result.passedColdContexts, "passed contexts");
  assertDecimal(result.excludedMockContexts, "mock contexts");
  ...
  assert(result.passedColdContexts === String(passing), "aggregate pass count");
...
export function attachHarnessObservations(
  ...
  assertSafeGraph(observations);
  for (const value of Object.values(counters))
    assert(Number.isSafeInteger(value) && value >= 0, "unsafe harness counter");
  const failed = counters.console > 0 || counters.page > 0 || counters.unhandled > 0 || counters.forbiddenNetwork > 0;
  const result: SanitizedProbeResult = {
    ...source,
...
export function controlledStructuralObservation(): SafeProbeObservation {
  return {
    ...
    wasmCriticalSections: [
      ...
      {
        criticalSectionClassification: "SDK_DECRYPTION",
        successfulInstantiateCount: 0,
        successfulInstantiateStreamingCount: 0,
        failedCallCount: 0,
      },
    ...
    transactionStatus: "NOT_RUN",
    transactionHash: null,
    transactionBlockNumber: null,
    resultHandleRead: false,
    authorizedDecryptionCompleted: false,
    authorizedPlaintextMatched: false,
    unauthorizedDecryptionAttempted: false,
    unauthorizedDecryptionRejected: false,
    forbiddenMaterialRetained: false,
