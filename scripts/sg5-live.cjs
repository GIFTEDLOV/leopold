...
function childEnv(extra) { return { PATH: process.env.PATH, HOME: process.env.HOME, NODE_ENV: "production", NEXT_TELEMETRY_DISABLED: "1", ...extra }; }
function protocolDigest() { require("ts-node/register/transpile-only"); return require(join(ROOT, "scripts/sg5-protocol.ts")).protocolDigest(); }
function loadVars() {
...
async function waitForHealth() {
  for (let attempt = 0; attempt < 180; attempt += 1) {
    try { const response = await fetch("http://127.0.0.1:3000/api/sg5-health"); if (response.ok) return; } catch {}
    await new Promise((resolvePromise) => setTimeout(resolvePromise, 500));
  }
  throw new Error("SG5_PRODUCTION_FRONTEND_HEALTH_TIMEOUT");
}
function stop(child) { if (!child || child.killed) return; try { process.kill(-child.pid, "SIGTERM"); } catch {} }
function writeEvidence(run, identity, digest) {
  const evidence = {
    ...
    environment: { chainId: "11155111", browser: "Chromium", productionBundle: true, walletModel: "TEST_AUTOMATION_WALLET_VIA_BROWSER_EIP1193_BRIDGE", frontendWalletModel: "PRODUCTION_WALLET_INTERFACE", secretsPersisted: false },
    probeDeployment: { chainId: "11155111", contractAddress: "0xfc672ca5846896A7A135943E79dd11283c38FE78", deploymentTransactionHash: "0x44b8701193ca8b0efd32a081786056af1a722fa0efb2ef1c92550687562090d4", deploymentBlockNumber: "11459741", compiler: "0.8.27" },
...
async function main() {
  ...
  if (!/^[0-9a-f]{40}$/u.test(expectedCommit || "") || !/^[0-9a-f]{40}$/u.test(expectedTree || "")) throw new Error("explicit preparation identity required");
  const identity = { branch: git("branch", "--show-current"), commit: git("rev-parse", "HEAD"), tree: git("rev-parse", "HEAD^{tree}") };
  if (identity.branch !== "main" || identity.commit !== expectedCommit || identity.tree !== expectedTree) throw new Error("SG5_PREPARATION_IDENTITY_MISMATCH");
  if (git("status", "--porcelain", "--untracked-files=all") !== "") throw new Error("SG5_PREPARATION_WORKTREE_NOT_CLEAN");
  const digest = protocolDigest(); if (digest !== expectedDigest) throw new Error("SG5_PROTOCOL_DIGEST_MISMATCH");
  ...
  clearOwnedPort();
  const runDirectory = join("/tmp", `zama-szn4-sg5-${process.pid}-${Date.now()}`); mkdirSync(runDirectory, { recursive: true, mode: 0o700 });
  const resultPath = join(runDirectory, "result.json"); const outputDir = join(runDirectory, "playwright-output"); mkdirSync(outputDir, { mode: 0o700 });
  const frontend = spawn(process.execPath, [NEXT, "start", "--hostname", "127.0.0.1", "--port", "3000"], { cwd: FRONTEND, env: childEnv({ SG5_PROBE_PAGE: PAGE_ENABLE, SG5_PROBE_PRODUCTION: "1", SG5_LIVE_ACK: ACK }), detached: true, stdio: "ignore" });
  try {
    await waitForHealth();
