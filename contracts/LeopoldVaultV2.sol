...
import {LeopoldSettlementBondEscrowV2} from "./LeopoldSettlementBondEscrowV2.sol";
import {ICompoundComet} from "./interfaces/ICompoundComet.sol";

interface ILeopoldWrapperUnwrapV2 {
    function unwrap(address from, address to, euint64 amount) external returns (bytes32);
}

/// @title Leopold V2 confidential prize-savings vault with persistent automatic entry
...
contract LeopoldVaultV2 is ZamaEthereumConfig, ReentrancyGuardTransient, IERC7984Receiver {
    ...
    LeopoldSettlementBondEscrowV2 public immutable SETTLEMENT_BOND_ESCROW;
    ...
    constructor(
        ...
        SETTLEMENT_BOND_ESCROW = new LeopoldSettlementBondEscrowV2(
            address(this),
    ...
    function requestStrategyAmount(uint256 epochId) external nonReentrant {
        ...
        if (epoch.kind == StrategyEpochKind.DEPLOY) {
            ...
            bytes32 requestId = ILeopoldWrapperUnwrapV2(address(ASSET)).unwrap(address(this), address(this), excess);
