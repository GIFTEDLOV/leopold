...
describe("keeper lifecycle gas policy", () => {
  ...
  it("accepts the reviewed two-participant allocation estimate with headroom", () => {
    const decision = applyGasPolicy("process-allocation", 2_269_439n);
    expect(decision.gasLimit).toBe(2_861_799n);
    expect(decision.gasLimit).toBeLessThanOrEqual(DEFAULT_GAS_POLICY.maximumGasLimit);
  });
